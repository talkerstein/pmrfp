-- ════════════════════════════════════════════════════════════════════
-- PMRFP — Geography guardrails foundation (P0)
-- Adds, purely additively (nothing dropped, nothing existing altered in a
-- breaking way):
--   • regions.status        — manual tier override (auto|active|founding|hidden)
--   • platform_settings.region_liquidity_threshold — tunable supply gate
--   • region_liquidity + region_category_liquidity VIEWS — computed supply per
--     region (reuses the has_active_trade_access join: paid + approved + active
--     trade/supplier orgs). Counts only — safe to expose like rfp_public.
--   • region_requests       — self-serve "request my area" (admin-approved; no
--                             anon writes to regions → no sprawl)
--   • regional_waitlist     — founding-region / no-supply capture (→ GHL)
--   • notification_preferences — real, enforceable prefs (settings toggles were
--                             previously UI-only smoke)
-- RLS mirrors existing patterns: contact_requests (public insert / admin read)
-- for the two public-writable tables; notifications (own read+write) for prefs.
-- ════════════════════════════════════════════════════════════════════

-- ── 1. regions.status (manual override; computed tier is primary) ───────
alter table public.regions
  add column if not exists status text not null default 'auto'
    check (status in ('auto','active','founding','hidden'));

-- ── 2. tunable liquidity threshold (single-row config) ──────────────────
alter table public.platform_settings
  add column if not exists region_liquidity_threshold integer not null default 3;

-- ── 3. computed liquidity views ─────────────────────────────────────────
-- "Paid, approved, active" listable org = the canonical supply unit. Mirrors
-- has_active_trade_access (org_type trade/supplier, not suspended, sub
-- active/comped) and additionally requires an approved public profile.
...
-- Counts only — no sensitive columns. Exposed like the rfp_public teaser view
-- so public surfaces (directory / region empty states) can show honest supply.
...
-- ── 4. region_requests (self-serve, admin-approved; sprawl-guarded) ─────
...
-- ── 5. regional_waitlist (founding-region / no-supply capture → GHL) ────
-- region_id nullable so it works BEFORE a region exists (pairs with a pending
-- region_request). reason distinguishes the capture surface.
...
-- ── 6. notification_preferences (real, enforceable) ─────────────────────
-- One row per user; per-type frequency. Replaces the UI-only settings toggles.
-- Send paths (cron + email) check these before dispatching.
